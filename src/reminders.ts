...
export async function ensureList(listName: string) {
  ...
  try {
    ...
    throw new Error(`Could not create list "${listName}". Please create it manually in Apple Reminders.`);
  }
}
...
function buildDateSetup(dueDate: Date, hasTime: boolean) {
  ...
  const props = hasTime ? ["due date:d", "remind me date:d"] : ["allday due date:d"];
