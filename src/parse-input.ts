...
function resolveDueDate(value: string, explicitTime: string | null, now: Date): ParsedDueDate | null {
  ...
  if (duration) {
    ...
    const target =
      duration.unit === "y"
        ? new Date(now.getFullYear() + duration.amount, now.getMonth(), now.getDate())
  ...
  const target = new Date(year, parseInt(dateMatch[2], 10) - 1, parseInt(dateMatch[1], 10), 0, 0, 0, 0);
...
export function parseDueDate(input: string, now: Date = new Date()): ParsedDueDate | null {
  const trimmed = input.trim();
  if (!trimmed) return null;

  // Split "2d 14:30" into value + optional time
  const match = trimmed.match(/^(\d+[smhdwy]|\d{1,2}\.\d{1,2}(?:\.\d{2,4})?)(?:\s+(\d{1,2}:\d{2}))?$/);
  if (!match) return null;
